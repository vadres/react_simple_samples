...
const HEAD_IMG = "https://static.thenounproject.com/png/58041-200.png"
const X_IMG    = "http://pngimage.net/wp-content/uploads/2018/06/icon-x-png-3.png"
...
const loadImage = (props) => {
  const { ctx, x, y, src, width, height } = props
  const img = new Image()
...
class Grido extends React.Component {
  ...
  handleTouch(event){
    let x = 0, pageX = event.changedTouches[0].pageX
                     - this.canvas.getBoundingClientRect().left
    let y = 0, pageY = event.changedTouches[0].pageY
                     - this.canvas.getBoundingClientRect().top
    
    for (let l = 0; l < GRID_SIZE; l++){
      for (let c = 0; c < GRID_SIZE; c++){
        if (pageX + 37 >= POS[l][c][0] &&
            pageY + 37 >= POS[l][c][1]) {
          x = l
          y = c
        }
    ...
    if(this.state.gameon && this.verifyArea(x, y) && 
        ((this.state.x + 1 == x && this.state.y == y) || 
         (this.state.x - 1 == x && this.state.y == y) ||
         (this.state.y + 1 == y && this.state.x == x) ||
         (this.state.y - 1 == y && this.state.x == x))){
      this.move(x,y)
    }
  }
